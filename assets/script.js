...
const pixStatusEndpoint = "../api/pix-status.php";
...
function applyPixPayment(payment) {
  ...
  currentPixPayment = {
    panel_payment_id: payment.panel_payment_id || payment.panel_pix_id || payment.id || "",
    panel_pix_id: payment.panel_pix_id || payment.panel_payment_id || payment.id || "",
    panel_reference: payment.panel_reference || payment.reference || payment.identifier || "",
    reference: payment.reference || payment.identifier || "",
...
async function notifyPixCopied() {
  if (pixCopyStatusSent || !currentPixPayment) return;

  pixCopyStatusSent = true;

  try {
    const response = await fetch(pixStatusEndpoint, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify(currentPixPayment),
      keepalive: true,
    });
